...
import ClientManagerWithTabs from '../clients/ClientManagerWithTabs'
import SuperAdminClientManager from '../clients/SuperAdminClientManager'
import SuperAdminControlPanel from '../admin/SuperAdminControlPanel'
import TeamManagementAgency from '../team/TeamManagementAgency'
import AutomationWorkflows from '../automation/AutomationWorkflows'
import MarketIntelligenceComponent from '../analytics/MarketIntelligence'
import AgencySettings from './AgencySettings'
import AdvancedAnalytics from '../analytics/AdvancedAnalytics'
import PredictiveAnalytics from '../analytics/PredictiveAnalytics'
import TaskAutomation from '../automation/TaskAutomation'
import QuickActions from '../../ui/QuickActions'
import AIAnalysisPanel from '../analytics/AIAnalysisPanel'
import ReviewResponder from '../reviews/ReviewResponder'
import ManualPostHelper from '../social/ManualPostHelper'
import ClientUserInvite from '../clients/ClientUserInvite'
import GoogleBusinessManager from '../business/GoogleBusinessManager'
import ClientImporter from '../clients/ClientImporter'
import { 
  EnhancedClient, 
...
export default function AgencyDashboard() {
  ...
  const loadDashboardData = async () => {
    ...
    try {
      ...
      const activeClientsCount = clients?.filter((c: EnhancedClient) => c.status === 'active' || !c.status).length || 0
      const avgScore = audits && audits.length > 0 
        ? audits.reduce((sum: number, audit: { overall_score?: number }) => sum + (audit.overall_score || 0), 0) / audits.length 
        : 0
      ...
      const clientsWithScores = clients?.map((client: EnhancedClient & { name?: string }) => {
        const recentAudit = audits?.find((a: { client_id?: string; overall_score?: number; created_at?: string }) => a.client_id === client.id)
        return {
          ...client,
      ...
      }).sort((a: EnhancedClient & { last_score?: number }, b: EnhancedClient & { last_score?: number }) => (b.last_score || 0) - (a.last_score || 0)).slice(0, 5) || []
      ...
      // Add recent audits
      audits?.slice(0, 3).forEach((audit: { id?: string; created_at?: string; client_id?: string; overall_score?: number }) => {
        if (audit.id && audit.created_at) {
          const clientName = clients?.find((c: EnhancedClient & { name?: string }) => c.id === audit.client_id)?.business_name || 'Unknown Client'
          activities.push({
            id: audit.id,
            type: 'audit',
      ...
      intelligence?.slice(0, 2).forEach((intel: MarketIntelligence) => {
  ...
  const handleClientCreated = async (client: EnhancedClient) => {
    ...
    <div className="min-h-screen bg-slate-900 flex">
      ...
      <div className="w-64 bg-slate-800 border-r border-slate-700 flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          {[
            ...
            { id: 'super-admin', name: 'Super Admin Control', show: isSuperAdmin }
      ...
      <main className="flex-1 overflow-auto">
                    ...
                    <div key={client.id} className="flex items-center justify-between p-3 hover:bg-slate-700 rounded-lg transition-colors">
                      ...
                      <div className="text-right">
                        <p className={`text-sm font-bold ${
                          (client as EnhancedClient & { last_score?: number }).last_score && (client as EnhancedClient & { last_score: number }).last_score >= 80 ? 'text-green-400' :
                          (client as EnhancedClient & { last_score?: number }).last_score && (client as EnhancedClient & { last_score: number }).last_score >= 60 ? 'text-yellow-400' :
                          'text-red-400'
                        }`}>
                          {(client as EnhancedClient & { last_score?: number }).last_score || 'N/A'}
                        </p>
                        <p className="text-slate-400 text-xs">
                          {(client as EnhancedClient & { last_audit_at?: string }).last_audit_at ? formatTimeAgo((client as EnhancedClient & { last_audit_at: string }).last_audit_at) : 'No audit'}
                        </p>
                      </div>
                    </div>
                  )) : (
        ...
        {activeTab === 'clients' && (isSuperAdmin || permissions?.can_create_clients || permissions?.can_edit_clients || permissions?.can_view_all_audits) && (
          (isSuperAdmin || membership?.role === 'owner') ? (
            <SuperAdminClientManager />
          ) : (
            <ClientManagerWithTabs />
          )
        )}
